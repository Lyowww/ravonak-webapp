...
export default function CheckoutPage() {
  ...
  useEffect(() => {
    ...
    void runPreview();
    // eslint-disable-next-line react-hooks/exhaustive-deps -- preview when selection loads
  }, [tgId, authStage, loading, addrId, recId]);

  if (authStage !== "verified" || tgId == null) {
    return (
      <div className="flex flex-1 flex-col bg-white px-4 pt-8">
        <PageHeader backHref="/market/cart" title="Оформление" />
        <p className="mt-6 text-[#949494]">Сначала войдите в аккаунт.</p>
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col bg-white pb-8">
      <PageHeader backHref="/market/cart" title="Оформление" />
